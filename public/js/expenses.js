...
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('fExpenseDate').value = todayStr();
  loadData();
  loadHeaderStats();

  ['searchMain', 'dateFrom', 'dateTo'].forEach(id => {
...
async function loadHeaderStats() {
  try {
    ...
    document.getElementById('totalAmount').textContent = fmtCurrency(d.totalExpenses);
    document.getElementById('totalCount').textContent  = d.expensesCount;
  } catch {}
}
...
function renderTable(rows, total) {
  ...
  tbody.innerHTML = rows.map((r, i) => `
    <tr>
      ...
      <td><strong class="text-dark">${esc(r.receiver_name)}</strong></td>
      <td><span class="badge bg-danger-subtle text-danger py-1 px-2 fw-semibold" style="font-size:.9rem">
        ${fmtCurrency(r.amount)}</span></td>
      <td class="d-none d-lg-table-cell">${esc(r.expense_reason) || '<span class="text-muted">—</span>'}</td>
...
function openAddModal() {
  ...
  document.getElementById('modalTitle').innerHTML =
    `<i class="bi bi-plus-circle-fill me-1"></i> ${t('modal_add_expense')}`;
...
async function editRecord(id) {
  try {
    ...
    document.getElementById('fReceiverName').value    = d.receiver_name;
    document.getElementById('fAmount').value          = d.amount;
    document.getElementById('fExpenseReason').value   = d.expense_reason  || '';
    document.getElementById('fReceiptNumber').value   = d.receipt_number;
    document.getElementById('fExpenseDate').value     = d.expense_date;
    ...
    document.getElementById('modalTitle').innerHTML =
      `<i class="bi bi-pencil-fill me-1"></i> ${t('modal_edit_expense')}`;
    new bootstrap.Modal(document.getElementById('mainModal')).show();
  } catch {
    showToast(t('err_server'), 'error');
  }
...
async function saveRecord() {
  ...
  const name   = document.getElementById('fReceiverName').value.trim();
  const amount = document.getElementById('fAmount').value;
  const rcpt   = document.getElementById('fReceiptNumber').value.trim();
  const date   = document.getElementById('fExpenseDate').value;

  if (!name)                { showToast(t('err_receiver_name'), 'error'); return; }
  ...
  fd.append('receiver_name',  name);
  fd.append('amount',         amount);
  fd.append('expense_reason', document.getElementById('fExpenseReason').value.trim());
  ...
  try {
    ...
    showToast(id ? t('msg_edit_expense') : t('msg_add_expense'), 'success');
    if (!id) currentPage = 1;
    loadData();
    loadHeaderStats();
  } catch {
...
async function deleteRecord(id) {
  ...
  const name    = record ? esc(record.receiver_name)  : `#${id}`;
  ...
  if (result.isConfirmed && result.value) {
    showToast(t('msg_delete_expense'), 'success');
